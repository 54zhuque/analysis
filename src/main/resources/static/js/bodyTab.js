...
layui.define(["element", "jquery"], function (exports) {
    ...
    Tab.prototype.render = function () {
        //显示左侧菜单
        var _this = this;
        var indexLi = '<li class="layui-nav-item layui-this"><a data-url="page/main.html">' +
            '<i class="layui-icon" data-icon=""></i><cite>成绩总览</cite></a></li>';
        $(".navBar ul").html(indexLi).append(_this.navBar(dataStr)).height($(window).height() - 210);
    ...
    $(".closePageOther").on("click", function () {
        if ($("#top_tabs li").length > 2 && $("#top_tabs li.layui-this cite").text() !== "成绩总览") {
            var menu = JSON.parse(window.sessionStorage.getItem("menu"));
        ...
        } else if ($("#top_tabs li.layui-this cite").text() === "成绩总览" && $("#top_tabs li").length > 1) {
            $("#top_tabs li").each(function () {
                if ($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")) {
                    element.tabDelete("bodyTab", $(this).attr("lay-id")).init();
